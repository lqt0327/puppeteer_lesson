...
(async () => {
  ...
  const queryData = async (url) => {
    ...
    const queryDetailData = async (url) => {
      ...
      let t = await page.waitForSelector('.muted')
      let title = await t.evaluate(el=>el.textContent)

      let d = await page.waitForSelector('blockquote')
      let desc = await d.evaluate(el=>el.innerHTML)
      page.close()
      return {
        title,
        desc
    ...
    for(let v of arr) {
      ...
      console.log('读取中：',href)

      let detail = await queryDetailData(href)
